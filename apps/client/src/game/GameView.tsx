...
export function GameView({ token, character, onLogout }: Props) {
  ...
  useEffect(() => {
    ...
    const keyDown=(e:KeyboardEvent)=>{ if(['KeyW','KeyA','KeyS','KeyD'].includes(e.code)){down.add(e.code);sendMovement();e.preventDefault();} if(['Digit1','Digit2','Digit3'].includes(e.code)){const index=Number(e.code.slice(-1))-1;const ability=latest.current?.abilities[index];if(ability)socketRef.current?.send({type:'cast',abilityId:ability.id,targetId:latest.current?.targetId ?? null});e.preventDefault();}};
    const keyUp=(e:KeyboardEvent)=>{if(['KeyW','KeyA','KeyS','KeyD'].includes(e.code)){down.delete(e.code);sendMovement();}};
    window.addEventListener('keydown',keyDown);window.addEventListener('keyup',keyUp);window.addEventListener('blur',()=>socketRef.current?.send({type:'move',x:0,z:0}));
  ...
  const now = world?.serverTime ?? Date.now(); const hpPct = world ? (world.self.hp/world.self.maxHp)*100 : 0; const targetVisible=world?.targetId===world?.monster.entityId && !world.monster.dead;
  ...
  return <main className="game-shell">
    ...
    {targetVisible && <section className="target-hud glass"><strong>{world.monster.name}</strong><div className="bar enemy"><i style={{width:`${(world.monster.hp/world.monster.maxHp)*100}%`}}/></div><small>{world.monster.hp} / {world.monster.maxHp}</small></section>}
    <button className="logout glass" onClick={onLogout}>Logout</button>
    <div className="hint glass">WASD Move · Click Slime / 1 Basic · 2–3 Skills</div>

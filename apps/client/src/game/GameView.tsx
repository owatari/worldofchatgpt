...
export function GameView({ token, character, onLogout }: Props) {
  ...
  const [world, setWorld] = useState<WorldUi | null>(null);
  ...
  const handleMessage = (message: ServerMessage, engine: GameEngine) => {
    ...
    engine.setSnapshot({
      self: message.self,
      players: message.type === 'state' ? message.players : [],
      monster: message.monster,
      selectedTargetId: message.selectedTargetId,
    });
  };
  ...
  useEffect(() => {
    ...
    const castBasicAt = (targetId: string) => {
      ...
      const basic = current?.abilities[0];
      if (!basic) return;
      socketRef.current?.send({ type: 'target', targetId });
      socketRef.current?.send({ type: 'cast', abilityId: basic.id, targetId });
    };
    const engine = new GameEngine(mount, castBasicAt);
    engineRef.current = engine;
    const socket = new GameSocket(token, (message) => handleMessage(message, engine), setStatus);
    socketRef.current = socket;
    socket.connect();
    return () => {
      socket.send({ type: 'move', x: 0, z: 0 });
      socket.close();
      engine.destroy();
  ...
  useEffect(() => {
    ...
    const sendMovement = () => {
      let strafe = 0;
      let forward = 0;
      if (down.has('KeyA')) strafe -= 1;
      if (down.has('KeyD')) strafe += 1;
      if (down.has('KeyW')) forward += 1;
      if (down.has('KeyS')) forward -= 1;
      const movement = engineRef.current?.getCameraRelativeMovement(strafe, forward) ?? { x: strafe, z: -forward };
      socketRef.current?.send({ type: 'move', x: movement.x, z: movement.z });
    };
    const keyDown = (event: KeyboardEvent) => {
      ...
      if (event.code === 'Tab') {
        const current = latest.current;
        if (current && !current.monster.dead) {
          const targetId = current.targetId === current.monster.entityId ? null : current.monster.entityId;
          socketRef.current?.send({ type: 'target', targetId });
          event.preventDefault();
        }
      }
      if (['Digit1', 'Digit2', 'Digit3'].includes(event.code)) {
        ...
        const ability = latest.current?.abilities[index];
        if (ability) {
          socketRef.current?.send({
            type: 'cast',
            abilityId: ability.id,
            targetId: latest.current?.targetId ?? null,
          });
        }
        event.preventDefault();
      }
    ...
    const keyUp = (event: KeyboardEvent) => {
      if (!movementCodes.includes(event.code)) return;
      down.delete(event.code);
      sendMovement();
    };
    const blur = () => {
      down.clear();
      socketRef.current?.send({ type: 'move', x: 0, z: 0 });
    };
    const movementTimer = window.setInterval(() => {
      if (down.size > 0) sendMovement();
    }, 50);
  ...
  return <main className="game-shell">
    ...
    <div className="hint glass">WASD Move · RMB Drag Camera · Wheel Zoom · Click/Tab Target · 1 Basic · 2–3 Skills</div>
    <section className="hotbar">{(world?.abilities ?? []).map((ability,index)=>{const remaining=Math.max(0,ability.readyAt-now);const pct=ability.cooldownMs?remaining/ability.cooldownMs:0;return <button key={ability.id} onClick={()=>cast(ability)} disabled={remaining>0 || Boolean(world?.self.dead)}><span className="key">{index+1}</span><b>{ability.name}</b>{remaining>0&&<span className="cooldown" style={{height:`${pct*100}%`}}/>}<em>{remaining>0?(remaining/1000).toFixed(1):''}</em></button>;})}</section>
